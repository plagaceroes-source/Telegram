...
async def main() -> None:
    ...
    async with session_scope() as session:
        for i, (posts_period, posts_sort, date_from, date_to) in enumerate(CASES):
            ...
            print(
                f"[case {i}] input=({posts_period!r}, {posts_sort!r}, {date_from!r}, {date_to!r}) => "
                f"resolved=({resolved_period!r}, {resolved_sort!r}, {resolved_from!r}, {resolved_to!r}) "
                f"count={len(top_posts)}"
            )
            for p in top_posts[:3]:
                print(f"[case {i}]   {p['published_at']} views={p['views']} reactions={p['reactions']} {p['snippet'][:40]!r}")
            sys.stdout.flush()
